...
/**
 * Écran 8 — Maison ÉCLORÉE, accueil (/ecloree). Gabarit e2e/portail.spec.ts.
 */
test.describe("Maison ÉCLORÉE — /ecloree", () => {
  test("affiche le hero et les rituels", async ({ page }) => {
    await page.goto("/ecloree");
    await expect(page.getByRole("heading", { name: "L'éclat vient de l'arbre.", level: 1 })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Trois gestes fondamentaux d'équilibre" })).toBeVisible();
  });
