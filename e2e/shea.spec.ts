...
/**
 * Écran 3 — Maison SHÉA, accueil (/shea). Gabarit e2e/portail.spec.ts.
 */
test.describe("Maison SHÉA — /shea", () => {
  test("affiche le hero et les Six Escales", async ({ page }) => {
    await page.goto("/shea");
    await expect(page.getByRole("heading", { name: "Le voyage commence à l'arbre.", level: 1 })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Les Six Escales" })).toBeVisible();
    await expect(page.getByRole("link", { name: "Découvrir la collection" })).toBeVisible();
  });
