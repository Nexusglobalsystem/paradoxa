...
/**
 * Écran 22 — Engagements & sourcing (/engagements). Gabarit
 * e2e/portail.spec.ts.
 */
test.describe("Engagements & sourcing — /engagements", () => {
  test("affiche les chiffres clés et les quatre piliers", async ({ page }) => {
    await page.goto("/engagements");
    await expect(page.getByRole("heading", { name: "De l'arbre au flacon", level: 1 })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Cinq étapes, de la canopée sahélienne au flacon" }),
    ).toBeVisible();
    await expect(page.getByRole("heading", { name: "Sourcing équitable & souveraineté sahélienne" })).toBeVisible();
  });
